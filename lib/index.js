...
function excerpt(options, fileOrDirToExcerptFrom) {
  if (!fileOrDirToExcerptFrom) return _excerpt(options);

  options = resolveOptions(options);
  var relativePath = path.relative(options.examplesDir, fileOrDirToExcerptFrom);
  options.examplesDir = path.resolve(options.examplesDir, relativePath);
  options.fragmentsDir = path.resolve(options.fragmentsDir,
    fs.lstatSync(options.examplesDir).isFile() ? path.dirname(relativePath) : relativePath);

  return options.clean ? del(path.join(options.fragmentsDir, '**'))
